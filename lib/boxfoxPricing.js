      ...
      if (q >= slab.minQty) {
        ...
        if (withinMax) {
          if (slab.price !== undefined && slab.price !== null && slab.price !== 0) {
            if (product?.extraDiscountAbove500 && q >= 500) {
              let disc = Number(slab.discount);
              if ((disc === undefined || disc === null || disc <= 0) && basePrice > 0) {
                disc = ((basePrice - Number(slab.price)) / basePrice) * 100;
              }
              if (disc > 0) {
    ...
    if (q >= tier.qty) {
      ...
      if (hasPrice || hasDiscount) {
        ...
        if (hasPrice) {
          ...
          if (product?.extraDiscountAbove500 && q >= 500 && basePrice > 0) {
