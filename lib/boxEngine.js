 ...
 */
export function calculateDynamicPrice(qty, p1, p10, p50, p100, p500, p1000, d10, d50, d100, d500, d1000, extraDiscountAbove500 = false) {
    const q = Math.max(1, parseFloat(qty) || 1);
    const basePrice = Math.max(0, parseFloat(p1) || 0);
    if (basePrice === 0) return 0;

    const tiers = [
        { qty: 1000, price: p1000, discount: d1000 },
        { qty: 500, price: p500, discount: d500 },
        { qty: 100, price: p100, discount: d100 },
        { qty: 50, price: p50, discount: d50 },
        { qty: 10, price: p10, discount: d10 }
    ];
    ...
    for (const tier of tiers) {
        if (q >= tier.qty) {
            ...
            if (hasPrice || hasDiscount) {
                if (hasDiscount) {
                    let disc = parseFloat(tier.discount);
                    if (extraDiscountAbove500 && q >= 500) {
                        disc = disc + (disc * disc / 100);
                    }
                    return Math.round(basePrice * (1 - disc / 100) * 100) / 100;
                }
                if (hasPrice) {
                    const pVal = parseFloat(tier.price);
                    // Legacy check: if discount is not set and price is <= 100, treat it as a discount percentage
                    if (pVal <= 100 && pVal > 0) {
                        let disc = pVal;
                        if (extraDiscountAbove500 && q >= 500) {
                            disc = disc + (disc * disc / 100);
                    ...
                    if (extraDiscountAbove500 && q >= 500 && basePrice > 0) {
                        let disc = ((basePrice - pVal) / basePrice) * 100;
                        disc = disc + (disc * disc / 100);
                        return Math.round(basePrice * (1 - disc / 100) * 100) / 100;
                    }
                    return pVal;
