...
export const generateInvoicePDF = (order) => {
    ...
    const drawBranding = () => {
        ...
        doc.setFontSize(11);
        doc.text(order.billingDetails?.isB2b ? 'B2B TAX INVOICE' : 'TAX INVOICE', 147, 34);
        doc.setFontSize(14);
    ...
    const isB2b = order.billingDetails?.isB2b;
    const recipientName = isB2b && order.billingDetails.companyName ? order.billingDetails.companyName : (order.customerName || 'PREMIUM CLIENT');

    doc.setFontSize(15);
    doc.setFont('helvetica', 'bold');
    doc.text(recipientName, 25, 92);
    ...
    let infoY = 109;
    if (isB2b && order.billingDetails.gstNumber) {
        doc.text(`GSTIN: ${order.billingDetails.gstNumber}`, 25, infoY);
        infoY += 5;
    }
    doc.text(`Site: ${order.shippingAddress || 'India Mainland'}`, 25, infoY);
    ...
    const isSameState = order.shippingAddress?.toLowerCase().includes('haryana');
    let totalBase = 0;
    let totalTax = 0;
    ...
    const tableData = order.items.map((item, idx) => {
        ...
        const inclusivePrice = parseFloat(item.price);
        const inclusiveTotal = inclusivePrice * qty;
        
        // 18% GST built-in calculation
        const basePrice = inclusivePrice / 1.18;
        const baseTotal = basePrice * qty;
        const taxTotal = inclusiveTotal - baseTotal;
        
        totalBase += baseTotal;
        totalTax += taxTotal;

        const taxLabel = isSameState ? '9% C/S' : '18% I';
        ...
        return [
            ...
            taxLabel,
            `INR ${inclusiveTotal.toFixed(2)}`
        ];
    });

    autoTable(doc, {
        startY: 125,
        head: [['#', 'INVENTORY SPECIFICATION', 'HSN', 'QTY', 'BASE RATE', 'TAX', 'TOTAL']],
        body: tableData,
        theme: 'striped',
        headStyles: { 
            fillColor: [17, 24, 39], 
    ...
    doc.setFontSize(8);
    doc.setTextColor(156, 163, 175);
    doc.text('TAXABLE VALUE (BASE)', summaryX, finalY + 5);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(17, 24, 39);
    doc.text(`INR ${totalBase.toFixed(2)}`, 190, finalY + 5, { align: 'right' });
    ...
    if (isSameState) {
        ...
        doc.setTextColor(17, 24, 39);
        doc.text(`INR ${(totalTax / 2).toFixed(2)}`, 190, currentY, { align: 'right' });
        currentY += 12;
        ...
        doc.setTextColor(17, 24, 39);
        doc.text(`INR ${totalTax.toFixed(2)}`, 190, currentY, { align: 'right' });
        currentY += 12;
    }
